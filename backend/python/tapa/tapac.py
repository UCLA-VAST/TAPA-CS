...
from tapa.split_kernel import split_kernel
...
def create_parser() -> argparse.ArgumentParser:
  ...
  group.add_argument(
      ...
      type=int, 
      dest='multi_fpga',
      help=('Use option to specify how many FPGAs to use. Without this argument, TAPA uses one FPGA.',)
  )
...
def main(argv: Optional[List[str]] = None):
  ...
  # if all_steps or args.run_floorplanning is not None and multi_fpga is not None:
  #   _logger.info("new step added ")
    # use multi-fpga to run floorplanning layer with new constraints
  ...
  if all_steps or args.run_floorplanning is not None:
    if args.floorplan_output is not None:
      ...
      if args.multi_fpga==None:
        ...
        v2s, slot_list = program.run_multi_fpga_floorplanning(
          _get_device_info(parser, args)['part_num'],
          args.multi_fpga,
          args.connectivity,
          args.floorplan_pre_assignments,    
