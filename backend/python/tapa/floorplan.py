...
from tapa.AutoBridge.src.autobridge.main import annotate_floorplan, annotate_inter_fpga_floorplan
...
def generate_floorplan(
  ...
  """
  _logger.info("mulit_fpga from floorplan")
  ...
  config_with_floorplan = annotate_floorplan(config, multi_fpga)

  return config, config_with_floorplan

def generate_inter_fpga_floorplan(
    part_num: str,
    multi_fpga: int,
    physical_connectivity: TextIO,
    read_only_args: List[str],
  ...
  get the pipeline level of each edge
  """
  _logger.info("Inter FPGA floorplan")
  config = get_floorplan_config(
      autobridge_dir,
      part_num,
      physical_connectivity,
  ...
  v2s, slot_list = annotate_inter_fpga_floorplan(config, multi_fpga)
