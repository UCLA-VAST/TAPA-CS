...
def add_invokes(input_file:str, file1, file2, device_1_items, device_2_items, program:tapa.core.Program):
    ...
    for line in lines:
        if '.invoke' in line:
            ...
            _logger.info(edge_1_name)
            _logger.info(type(edge_1_name))
            if 'ASYNC_MMAP_VERTEX_'+edge_1_name+'__m_axi' in device_1_names:
                _logger.info("hello")
            if task_name in device_1_names and 'ASYNC_MMAP_VERTEX_'+edge_1_name+'__m_axi' in device_1_names or task_name in device_1_names and edge_1_name in middle_streams:
                file1.write(line)
                _logger.info(task_name)
                device_1_names = device_1_names.replace('TASK_VERTEX_'+task_name, '',1)
                _logger.info(device_1_names)
            elif task_name in device_2_names and 'ASYNC_MMAP_VERTEX_'+edge_1_name+'__m_axi' in device_2_names or task_name in device_2_names and edge_1_name in middle_streams:
                file2.write(line)
                device_2_names = device_2_names.replace('TASK_VERTEX_'+task_name, '',1)
