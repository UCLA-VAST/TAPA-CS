...
from tapa.AutoBridge.src.autobridge.Opt.DataflowGraph import Vertex
...
from tapa.AutoBridge.src.autobridge.util import get_mip_model_silent
...
class Bipartition:
  ...
  def get_bipartition(
    ...
    """
    _logger.info("hello from inside bipartition")
    m = get_mip_model_silent()

    v2var = self._create_ilp_vars(m)
    _logger.info("starting add_opt_goal")
    self._add_opt_goal(m, v2var, direction)
    _logger.info("done with add_opt_goal")
  ...
  def _add_opt_goal(
    ...
    for e, cost_var in e2cost_var.items():
      _logger.info(cost_var)
      _logger.info(e.name)
      _logger.info(e.src.name)
      _logger.info(e.dst.name)
      _logger.info(_get_loc_after_partition(e.src) - _get_loc_after_partition(e.dst))
      _logger.info(_get_loc_after_partition(e.dst) - _get_loc_after_partition(e.src))
      _logger.info(cost_var >= _get_loc_after_partition(e.src) - _get_loc_after_partition(e.dst))
      _logger.info(cost_var >= _get_loc_after_partition(e.dst) - _get_loc_after_partition(e.src))
      _logger.info("end one edge, cost_var")
...
class Bipartition_multiFPGA:
  ...
  def _add_opt_goal(
    ...
    for e, cost_var in e2cost_var.items():
      ...
      # _logger.info(e.src.name)
      # _logger.info(e.dst.name)
      num1 = _get_loc_after_partition(e.src) - _get_loc_after_partition(e.dst)
      num2 = _get_loc_after_partition(e.dst) - _get_loc_after_partition(e.src)
      num3 = cost_var >= _get_loc_after_partition(e.src) - _get_loc_after_partition(e.dst)
      num4 = cost_var >= _get_loc_after_partition(e.dst) - _get_loc_after_partition(e.src)
      # _logger.info(num1)
      # _logger.info(num2)
      # _logger.info(num3)
      # _logger.info(num4)
