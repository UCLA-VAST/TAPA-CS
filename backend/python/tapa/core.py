...
class Program:
  ...
  def run_floorplanning(
    ...
    return self
  def run_multi_fpga_floorplanning(
      self,
      part_num,
      multi_fpga,
      connectivity: TextIO,
      ...
      read_only_args: List[str] = [],
      write_only_args: List[str] = [],
      **kwargs,)-> Tuple[Dict[Vertex, Slot], List[Slot]]:
    _logger.info('Running Inter-FPGA floorplanning')
    os.makedirs(self.autobridge_dir, exist_ok=True)

    # generate partitioning constraints if partitioning directive is given
    v2s, slot_list = generate_inter_fpga_floorplan(
        part_num,
        multi_fpga,
        connectivity,
        read_only_args,
  ...
  def _connect_fifos(self, task: Task) -> None:
    _logger.info("  connecting %s's children tasks", task.name)
  ...
  def _instantiate_fifos(
    ...
    _logger.info('  instantiating FIFOs in %s', task.name)
    ...
    for fifo_name, fifo in fifos.items():
      ...
      for suffixes, fmt, fifo_tag in zip(
          ...
          ('info: R: {fifo_prefix}{fifo:>{width}}{fifo_suffix} -> '
           '{task_prefix}{task:<{width}}{task_suffix} %h',
           'info: W: {task_prefix}{task:>{width}}{task_suffix} -> '
      ...
      task.module.add_logics(infoging_blocks)
  ...
  def _instantiate_children_tasks(
    ...
    addr_width = 64
    _logger.info('Set the address width of async_mmap to %d', addr_width)
