...
typedef struct {
    ...
    XVecadd_1_uio_map maps[ MAX_UIO_MAPS ];
} XVecadd_1_uio_info;

/***************** Variable Definitions **************************************/
static XVecadd_1_uio_info uio_info;
...
static int uio_info_read_name(XVecadd_1_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XVecadd_1_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XVecadd_1_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XVecadd_1_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XVecadd_1_Initialize(XVecadd_1 *InstancePtr, const char* InstanceName) {
	XVecadd_1_uio_info *InfoPtr = &uio_info;
