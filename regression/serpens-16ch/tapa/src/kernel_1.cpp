...
void Stream2Mmap_1(tapa::istream<int> &yvec, tapa::mmap<int> temps_1, int NUM_ITE, int P_N)
{   
    const int rp_time = (P_N == 0)? 1 : P_N;
    const int ite_num = rp_time * (NUM_ITE + 1) + 3;
...
void Stream2Mmap_2(tapa::istream<MultXVec> &axvec, tapa::mmap<MultXVec> temps_2, int NUM_A_LEN, int P_N)
{
    const int rp_time = (P_N == 0)? 1 : P_N;
    const int ite_num = rp_time * NUM_A_LEN;
...
void Mmap2Stream_1(tapa::mmap<int> temps_1, tapa::mmap<int> temps_2,tapa::mmap<int> temps_3, tapa::mmap<int> temps_4, tapa::mmap<int> temps_5, tapa::mmap<int> temps_6,
tapa::mmap<int> temps_7, tapa::mmap<int> temps_8, tapa::mmap<int> temps_9, tapa::mmap<int> temps_10, tapa::mmap<int> temps_11, tapa::mmap<int> temps_12,
tapa::mmap<int> temps_13,tapa::mmap<int> temps_14, tapa::mmap<int> temps_15, tapa::mmap<int> temps_16, int NUM_ITE, int P_N, tapa::ostream<int> &out_board_1)
{
	const int rp_time = (P_N == 0)? 1 : P_N;
	const int ite_num = rp_time * (NUM_ITE + 1) + 3;
        ...
        {
            out_board_1<<temps_1[j];
            out_board_1<<temps_2[j];
            out_board_1<<temps_3[j];
            out_board_1<<temps_4[j];
            out_board_1<<temps_5[j];
            out_board_1<<temps_6[j];
            out_board_1<<temps_7[j];
            out_board_1<<temps_8[j];
            out_board_1<<temps_9[j];
            out_board_1<<temps_10[j];
            out_board_1<<temps_11[j];
            out_board_1<<temps_12[j];
            out_board_1<<temps_13[j];
            out_board_1<<temps_14[j];
            out_board_1<<temps_15[j];
            out_board_1<<temps_16[j];
        }
}

void Mmap2Stream_2(tapa::mmap<MultXVec> temps_1, tapa::mmap<MultXVec> temps_2, tapa::mmap<MultXVec> temps_3, tapa::mmap<MultXVec> temps_4, tapa::mmap<MultXVec> temps_5, tapa::mmap<MultXVec> temps_6, tapa::mmap<MultXVec> temps_7, tapa::mmap<MultXVec> temps_8, tapa::mmap<MultXVec> temps_9, tapa::mmap<MultXVec> temps_10,tapa::mmap<MultXVec> temps_11,tapa::mmap<MultXVec> temps_12,tapa::mmap<MultXVec> temps_13,tapa::mmap<MultXVec> temps_14,tapa::mmap<MultXVec> temps_15,tapa::mmap<MultXVec> temps_16,int NUM_A_LEN, int P_N, tapa::ostream<MultXVec> &out_board_2)
{
        const int rp_time = (P_N == 0)? 1 : P_N;
	const int ite_num = rp_time * NUM_A_LEN;
	...
	for(int j=0;j<ite_num;++j)
        {
            out_board_2<<temps_1[j];
            out_board_2<<temps_2[j];
            out_board_2<<temps_3[j];
            out_board_2<<temps_4[j];
            out_board_2<<temps_5[j];
            out_board_2<<temps_6[j];
            out_board_2<<temps_7[j];
            out_board_2<<temps_8[j];
            out_board_2<<temps_9[j];
            out_board_2<<temps_10[j];
            out_board_2<<temps_11[j];
            out_board_2<<temps_12[j];
            out_board_2<<temps_13[j];
            out_board_2<<temps_14[j];
            out_board_2<<temps_15[j];
            out_board_2<<temps_16[j];
        }
}
...
void Serpens(tapa::mmap<int> edge_list_ptr,
             tapa::mmaps<int, NUM_CH_SPARSE> temps_1,
             tapa::mmaps<MultXVec, NUM_CH_SPARSE> temps_2,
             ...
             tapa::ostream<int> out_board_1,
             tapa::ostream<MultXVec> out_board_2,
             tapa::istream<int> in_board_1,
             tapa::istream<int> in_board_2,
             const int NUM_ITE,
             const int NUM_A_LEN,
             const int M,
             const int K,
    ...
    /* =========deploy modules======= */
    // tapa::mmaps<ap_uint<512>, NUM_CH_SPARSE> temps_1("temps_1");
    // tapa::mmaps<MultXVec, NUM_CH_SPARSE> temps_2("temps_2");
    tapa::task()
        ...
        .invoke<tapa::join, NUM_CH_SPARSE>(Stream2Mmap_1, Yvec_inst, temps_1, NUM_ITE, P_N)
        .invoke<tapa::join, NUM_CH_SPARSE>(Stream2Mmap_2, fifo_aXvec, temps_2, NUM_A_LEN, P_N)
        .invoke<tapa::join>(Mmap2Stream_1, temps_1[0],temps_1[1], temps_1[2], temps_1[3], temps_1[4], temps_1[5], temps_1[6],  temps_1[7],  temps_1[8],  temps_1[9],  temps_1[10],  temps_1[11],  temps_1[12],  temps_1[13],  temps_1[14],  temps_1[15],  NUM_ITE, P_N, out_board_1)
        .invoke<tapa::join>(Mmap2Stream_2, temps_2[0],temps_2[1], temps_2[2], temps_2[3], temps_2[4], temps_2[5], temps_2[6],  temps_2[7],  temps_2[8],  temps_2[9],  temps_2[10],  temps_2[11],  temps_2[12],  temps_2[13],  temps_2[14],  temps_2[15],  NUM_A_LEN, P_N, out_board_2)
        // .invoke<tapa::join>(Mmap2Stream_2, temps_2, NUM_A_LEN, out_board_2)
