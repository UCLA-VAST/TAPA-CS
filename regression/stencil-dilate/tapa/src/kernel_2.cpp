...
void Mmap2Stream(tapa::mmap<INTERFACE_WIDTH> mmap, uint64_t n,
                 tapa::ostream<INTERFACE_WIDTH>& stream) {
  for (uint64_t i = 0; i < n; ++i) {
    stream << mmap[i];
  }
}
void Stream2Mmap(tapa::istream<INTERFACE_WIDTH>& stream, tapa::mmap<INTERFACE_WIDTH> mmap,
                 uint64_t n) {
  for (uint64_t i = 0; i < n; ++i) {
    stream >> mmap[i];
  }
...
void unikernel_2(tapa::mmap<INTERFACE_WIDTH> in_8,
tapa::mmap<INTERFACE_WIDTH> out_8,
tapa::mmap<INTERFACE_WIDTH> in_9,
tapa::mmap<INTERFACE_WIDTH> out_9,
tapa::mmap<INTERFACE_WIDTH> in_10,
tapa::mmap<INTERFACE_WIDTH> out_10,
tapa::mmap<INTERFACE_WIDTH> in_11,
tapa::mmap<INTERFACE_WIDTH> out_11,
tapa::mmap<INTERFACE_WIDTH> in_12,
tapa::mmap<INTERFACE_WIDTH> out_12,
tapa::mmap<INTERFACE_WIDTH> in_13,
tapa::mmap<INTERFACE_WIDTH> out_13,
tapa::mmap<INTERFACE_WIDTH> in_14,
tapa::mmap<INTERFACE_WIDTH> out_14,
uint32_t iters,
...
tapa::task()
.invoke(DILATE,  in_board,  k_wr[7],  iters)
.invoke(Stream2Mmap, k_wr[7], temp, 3)
...
// .invoke(Stream2Mmap, k_wr[14], temp, 3)
// .invoke(Mmap2Stream, temp, 3, out_board)
;
}