...
void Scatter_1_2(tapa::istream<ap_uint<512>> & fifo_in,
                 tapa::ostream<ap_uint<256>> & fifo_out_1,
                 tapa::ostream<ap_uint<256>> &fifo_out_2) {
    for (;;) {
#pragma HLS pipeline II=1
        bool flag_nop = fifo_in.empty();
        // for (int i = 0; i < 2; ++i) {
            flag_nop |= fifo_out_1.full();
            flag_nop |= fifo_out_2.full();
        // }
        if (!flag_nop) {
            ap_uint<512> tmp = fifo_in.read();
            // for (int i = 0; i < 2; ++i) {
                fifo_out_1.write(tmp(255 + 0 * 256, 0 * 256));
                fifo_out_2.write(tmp(255 + 1 * 256, 1 * 256));
            // }
        }
    }
}
...
void Sextans(tapa::mmap<int> edge_list_ptr,
    ...
    //  tapa::streams<ap_uint<256>, 2, FIFO_DEPTH> fifo_A_pe_input=(fifo_A_pe[0], fifo_A_pe[1])
    tapa::task()
    .invoke(read_edge_list_ptr,NUM_ITE,M,P_N,K,edge_list_ptr,fifo_edge_list_ptr,PE_inst)

    // .invoke<tapa::join, NUM_CH_SPARSE>(read_A,edge_list_ch,fifo_A,NUM_A_LEN,P_N)
    .invoke(read_A, edge_list_ch[0], fifo_A[0], NUM_A_LEN, P_N)
    .invoke(read_A, edge_list_ch[1], fifo_A[1], NUM_A_LEN, P_N)
    .invoke(read_A, edge_list_ch[2], fifo_A[2], NUM_A_LEN, P_N)
    .invoke(read_A, edge_list_ch[3], fifo_A[3], NUM_A_LEN, P_N)
    .invoke(read_A, edge_list_ch[4], fifo_A[4], NUM_A_LEN, P_N)
    .invoke(read_A, edge_list_ch[5], fifo_A[5], NUM_A_LEN, P_N)
    .invoke(read_A, edge_list_ch[6], fifo_A[6], NUM_A_LEN, P_N)
    .invoke(read_A, edge_list_ch[7], fifo_A[7], NUM_A_LEN, P_N)

   
    // .invoke<tapa::detach, NUM_CH_SPARSE>(Scatter_1_2,fifo_A,fifo_A_pe)
    .invoke<tapa::detach>(Scatter_1_2,fifo_A[0],fifo_A_pe[0], fifo_A_pe[1])
    .invoke<tapa::detach>(Scatter_1_2,fifo_A[1],fifo_A_pe[2], fifo_A_pe[3])
    .invoke<tapa::detach>(Scatter_1_2,fifo_A[2],fifo_A_pe[4], fifo_A_pe[5])
    .invoke<tapa::detach>(Scatter_1_2,fifo_A[3],fifo_A_pe[6], fifo_A_pe[7])
    .invoke<tapa::detach>(Scatter_1_2,fifo_A[4],fifo_A_pe[8], fifo_A_pe[9])
    .invoke<tapa::detach>(Scatter_1_2,fifo_A[5],fifo_A_pe[10], fifo_A_pe[11])
    .invoke<tapa::detach>(Scatter_1_2,fifo_A[6],fifo_A_pe[12], fifo_A_pe[13])
    .invoke<tapa::detach>(Scatter_1_2,fifo_A[7],fifo_A_pe[14], fifo_A_pe[15])

    .invoke<tapa::join, NUM_CH_B>(read_B,mat_B_ch,fifo_B_pe,K,P_N)
    // .invoke(read_B, mat_B_ch[0],fifo_B_pe[0], K, P_N) 
    // .invoke(read_B, mat_B_ch[1], fifo_B_pe[1], K, P_N)
    // .invoke(read_B, mat_B_ch[2], fifo_B_pe[2], K, P_N)
    // .invoke(read_B, mat_B_ch[3], fifo_B_pe[3], K, P_N)
    .invoke<tapa::join, NUM_CH_SPARSE * PEG_PER_A>(PEG_Bmtx,PE_inst,fifo_edge_list_ptr,fifo_A_pe,fifo_B_pe,PE_inst,fifo_edge_list_ptr,fifo_B_pe,PE_inst_to_Cmtx,fifo_edge_list_ptr_to_Cmtx,fifo_aBvec)

    .invoke<tapa::join, NUM_CH_SPARSE * PEG_PER_A>(PEG_Cmtx,PE_inst_to_Cmtx,fifo_edge_list_ptr_to_Cmtx,fifo_aBvec,fifo_C_pe)
    ...
    // .invoke<tapa::join, NUM_CH_C>(read_C,mat_C_ch_in,fifo_C_read_in,M,P_N,wrC_inst)
    .invoke(read_C, mat_C_ch_in[0], fifo_C_read_in[0], M, P_N, wrC_inst)
    .invoke(read_C, mat_C_ch_in[1], fifo_C_read_in[1], M, P_N, wrC_inst)
    .invoke(read_C, mat_C_ch_in[2], fifo_C_read_in[2], M, P_N, wrC_inst)
    .invoke(read_C, mat_C_ch_in[3], fifo_C_read_in[3], M, P_N, wrC_inst)
    .invoke(read_C, mat_C_ch_in[4], fifo_C_read_in[4], M, P_N, wrC_inst)
    .invoke(read_C, mat_C_ch_in[5], fifo_C_read_in[5], M, P_N, wrC_inst)
    .invoke(read_C, mat_C_ch_in[6], fifo_C_read_in[6], M, P_N, wrC_inst)
    .invoke(read_C, mat_C_ch_in[7], fifo_C_read_in[7], M, P_N, wrC_inst)
    // .invoke<tapa::detach, NUM_CH_C>(FloatvMultConst,  beta_u,  fifo_C_read_in,  fifo_C_read_in_beta  )
    .invoke<tapa::detach>(FloatvMultConst,  beta_u,  fifo_C_read_in[0],  fifo_C_read_in_beta[0]  )
    .invoke<tapa::detach>(FloatvMultConst,  beta_u,  fifo_C_read_in[1],  fifo_C_read_in_beta[1]  )
    .invoke<tapa::detach>(FloatvMultConst,  beta_u,  fifo_C_read_in[2],  fifo_C_read_in_beta[2]  )
    .invoke<tapa::detach>(FloatvMultConst,  beta_u,  fifo_C_read_in[3],  fifo_C_read_in_beta[3]  )
    .invoke<tapa::detach>(FloatvMultConst,  beta_u,  fifo_C_read_in[4],  fifo_C_read_in_beta[4]  )
    .invoke<tapa::detach>(FloatvMultConst,  beta_u,  fifo_C_read_in[5],  fifo_C_read_in_beta[5]  )
    .invoke<tapa::detach>(FloatvMultConst,  beta_u,  fifo_C_read_in[6],  fifo_C_read_in_beta[6]  )
    .invoke<tapa::detach>(FloatvMultConst,  beta_u,  fifo_C_read_in[7],  fifo_C_read_in_beta[7]  )
    // .invoke<tapa::detach, NUM_CH_C>(FloatvMultConst,  alpha_u,  fifo_C_ch_result,  fifo_C_ch_result_alpha  )
    .invoke<tapa::detach>(FloatvMultConst,  alpha_u,  fifo_C_ch_result[0],  fifo_C_ch_result_alpha[0]  )
    .invoke<tapa::detach>(FloatvMultConst,  alpha_u,  fifo_C_ch_result[1],  fifo_C_ch_result_alpha[1]  )
    .invoke<tapa::detach>(FloatvMultConst,  alpha_u,  fifo_C_ch_result[2],  fifo_C_ch_result_alpha[2]  )
    .invoke<tapa::detach>(FloatvMultConst,  alpha_u,  fifo_C_ch_result[3],  fifo_C_ch_result_alpha[3]  )
    .invoke<tapa::detach>(FloatvMultConst,  alpha_u,  fifo_C_ch_result[4],  fifo_C_ch_result_alpha[4]  )
    .invoke<tapa::detach>(FloatvMultConst,  alpha_u,  fifo_C_ch_result[5],  fifo_C_ch_result_alpha[5]  )
    .invoke<tapa::detach>(FloatvMultConst,  alpha_u,  fifo_C_ch_result[6],  fifo_C_ch_result_alpha[6]  )
    .invoke<tapa::detach>(FloatvMultConst,  alpha_u,  fifo_C_ch_result[7],  fifo_C_ch_result_alpha[7]  )
    // .invoke<tapa::detach, NUM_CH_C>(FloatvAddFloatv,  fifo_C_ch_result_alpha,  fifo_C_read_in_beta,  fifo_C_ch  )
    .invoke<tapa::detach>(FloatvAddFloatv,  fifo_C_ch_result_alpha[0],  fifo_C_read_in_beta[0],  fifo_C_ch[0] )
    .invoke<tapa::detach>(FloatvAddFloatv,  fifo_C_ch_result_alpha[1],  fifo_C_read_in_beta[1],  fifo_C_ch[1] )
    .invoke<tapa::detach>(FloatvAddFloatv,  fifo_C_ch_result_alpha[2],  fifo_C_read_in_beta[2],  fifo_C_ch[2] )
    .invoke<tapa::detach>(FloatvAddFloatv,  fifo_C_ch_result_alpha[3],  fifo_C_read_in_beta[3],  fifo_C_ch[3] )
    .invoke<tapa::detach>(FloatvAddFloatv,  fifo_C_ch_result_alpha[4],  fifo_C_read_in_beta[4],  fifo_C_ch[4] )
    .invoke<tapa::detach>(FloatvAddFloatv,  fifo_C_ch_result_alpha[5],  fifo_C_read_in_beta[5],  fifo_C_ch[5] )
    .invoke<tapa::detach>(FloatvAddFloatv,  fifo_C_ch_result_alpha[6],  fifo_C_read_in_beta[6],  fifo_C_ch[6] )
    .invoke<tapa::detach>(FloatvAddFloatv,  fifo_C_ch_result_alpha[7],  fifo_C_read_in_beta[7],  fifo_C_ch[7] )
    // .invoke<tapa::join, NUM_CH_C>(write_C,wrC_inst,fifo_C_ch,mat_C_ch)
    .invoke(write_C,wrC_inst[0],fifo_C_ch[0],mat_C_ch[0])
    .invoke(write_C,wrC_inst[1],fifo_C_ch[1],mat_C_ch[1])
    .invoke(write_C,wrC_inst[2],fifo_C_ch[2],mat_C_ch[2])
    .invoke(write_C,wrC_inst[3],fifo_C_ch[3],mat_C_ch[3])
    .invoke(write_C,wrC_inst[4],fifo_C_ch[4],mat_C_ch[4])
    .invoke(write_C,wrC_inst[5],fifo_C_ch[5],mat_C_ch[5])
    .invoke(write_C,wrC_inst[6],fifo_C_ch[6],mat_C_ch[6])
    .invoke(write_C,wrC_inst[7],fifo_C_ch[7],mat_C_ch[7])
