...
void Stream2Mmap_out(tapa::istream<pkt> &L1_out_dist, uint64_t 10, tapa::mmap<pkt> &temp1)
{
...
void Stream2Mmap_out_id(tapa::istream<id_pkt> &L1_out_id, uint64_t 10, tapa::mmap<pkt> &temp1)
{
    for(uint64_t i=0;i<10;++i)
    {
        L1_out_id>>temp1[i];
...
void Stream2Mmap_in(tapa::istream<pkt> &in_board_1, tapa::mmap<pkt> temp9, tapa::mmap<pkt> temp10)
{
    for(uint64_t i=0;i<10;++i)
    {
        in_board_1>>temp9[i];
...
void Stream2Mmap_id(tapa::istream<pkt_id> &in_board_2, tapa::mmap<pkt_id> temp9, tapa::mmap<pkt_id> temp10)
{
    for(uint64_t i=0;i<10;++i)
    {
        in_board_1>>temp9[i];
...
void Mmap2Stream_in(tapa::mmap<pkt> &temp9, tapa::mmap<pkt> &temp10, tapa::ostream<pkt> &in_board_1, tapa::ostream<pkt> &in_board_2)
{
    for(uint64_t i=0;i<10;++i)
    {
        in_board_1<<temp9[i];
...
void Mmap2Stream_id(tapa::mmap<pkt_id> &temp9, tapa::mmap<pkt_id> &temp10, tapa::ostream<pkt_id> &in_board_1, tapa::ostream<pkt_id> &in_board_2)
{
    for(uint64_t i=0;i<10;++i)
    {
        in_board_1<<temp9[i];
...
void Knn_1(tapa::mmap<INTERFACE_WIDTH> in_0,
...
tapa::mmap<INTERFACE_WIDTH> L3_out_dist,
tapa::mmap<INTERFACE_WIDTH> L3_out_id,
tapa::mmap<pkt> temp1,
tapa::mmap<pkt_id> temp2, 
tapa::mmap<pkt> temp3,
tapa::mmap<pkt_id> temp4, 
tapa::mmap<pkt> temp5,
tapa::mmap<pkt_id> temp6,
tapa::mmap<pkt> temp7,
tapa::mmap<pkt_id> temp8,
tapa::mmap<pkt> temp9,
tapa::mmap<pkt> temp10,
tapa::mmap<pkt_id> temp11,
tapa::mmap<pkt> temp12,
tapa::ostream<pkt> &out_board_1,
tapa::istream<pkt_id> &out_board_2,
tapa::istream<pkt> &in_board_1,
tapa::istream<pkt_id> &in_board_1){
...
tapa::task()
    ...
    .invoke(Stream2Mmap_out, L1_out_dist[1], 10, temp3)
    .invoke(Stream2Mmap_out_id, L1_out_id[1], 10, temp4)
    ...
    .invoke(Mmap2Stream_in, temp9, temp10, in_board_1, in_board_2)
    .invoke(Mmap2Stream_id, temp11, temp12, in_board_3, in_board_4)
    .invoke( krnl_globalSort_L3,  in_board_1,//input,
      in_board_3, //input
        in_board_2, //input 
         in_board_4, //input
