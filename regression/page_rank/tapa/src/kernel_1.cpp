...
void Control(Pid num_partitions, tapa::mmap<uint64_t> metadata,
             ...
             // to UpdateHandler
             tapa::ostream<Eid>& update_config_q,
             tapa::ostream<bool>& update_phase_q,
             // from UpdateHandler
             tapa::istream<NumUpdates>& num_updates_q,
             // to ProcElem
             tapa::ostream<TaskReq>& task_req_q,
             // from ProcElem
             tapa::istream<TaskResp>& task_resp_q, 
             int pe) {
  // HLS crashes without this...
  // for (int pe = 0; pe < kNumPes; ++pe) {
// #pragma HLS unroll
    update_config_q.close();
    update_phase_q.close();
    task_req_q.close();
  // }
...
config_update_offsets:
  for (Pid pid = 0; pid < num_partitions; ++pid) {
    auto update_offset = metadata[2 + num_partitions+ pid];
    VLOG_F(8, info) << "update offset[" << pid << "]: " << update_offset;
    update_config_q[pid].write(update_offset);
  }
...
// #pragma HLS unroll
        TaskReq req{
            ...
            /* num_edges        = */ num_edges_local[pid],
            /* vid_offset       = */ vid_offset,
            /* eid_offset       = */ eid_offsets[pid],
            /* init             = */ 0.f,   // unused
...
#pragma HLS dependence false variable = num_updates_local
      ...
      // for (int pe = 0; pe < kNumPes; ++pe) {
// #pragma HLS unroll
        if (!done && num_updates_q.try_read(num_updates)) {
          done |= true;
...
// #pragma HLS unroll
        ...
        if (pid < num_partitions) {
          TaskReq req{TaskReq::kGather,
                      ...
                      false};
          if (task_req_q.try_write(req)) ++pid_send[pe];
        }
      }
...
void PageRank_1(Pid num_partitions, tapa::mmap<uint64_t> metadata,
...
.invoke(UpdateHandler,  "UpdateHandler",  num_partitions, update_config[3],  update_phase[3],  num_updates[3], update_req[3],  update_pe2mm[3],  update_mm2pe[3], update_read_addr[3],  update_read_data[3], update_write_addr[3],  update_write_data[3])
